    ...
    element-loading-text="拼命加载中">
        ...
        <div class="container">
            ...
            <el-table :data="tableData" border class="table" ref="multipleTable" @selection-change="handleSelectionChange">
                ...
                <el-table-column prop="user_phone" label="手机号码" width="130">
                </el-table-column>
                <el-table-column prop="email" label="邮箱" width="200">
                </el-table-column>
                <el-table-column prop="user_status" label="人员状态" :formatter="formatter" sortable width="110">
                ...
                <el-table-column prop="is_active" label="是否可用" :formatter="formatter" sortable width="110">
                    <template slot-scope='scope'>
                ...
                <el-table-column prop="roles" label="拥有的角色" sortable width="150" > 
                    <template slot-scope="scope" >
                      <span v-for="(item ,n) in scope.row.roles" :key="n" >[{{item.name}}], </span>
                    </template>
                </el-table-column>
                <el-table-column prop="create" label="创建时间" sortable width="150">
                </el-table-column>

                <el-table-column label="操作" width="220" align="center" fixed="right">
                    <template slot-scope="scope">
                        <el-button type="text"  icon="el-icon-edit" @click="editRoles(scope.row)">设置角色</el-button>
        ...
        <!-- 修改用户角色弹出框 -->
        <el-dialog title="编辑" :visible.sync="editRolesVisible" width="30%" >
          <div class="checkRoles">
            <el-checkbox-group v-model="checkRolesList" size="small">
...
export default {
  ...
  data() {
    return {
      ...
      delType: "", // allDel:批量删除    del:单条删除
      checkRolesList: [], //修改用户所拥有的角色
      allRoles: [], //获取所有角色
      curUserRoles: [],
      form: {
        username: "",
  ...
  methods: {
    ...
    async saveEditRoles() {
      let that = this;
      console.log(that.row.id);

      console.log(that.checkRolesList);
      let formData = {
        profileId: that.row.id,
        roleIdList: that.checkRolesList
      };
      let allRoles = await this.$post(
        that.$path + "users/usersRole/",
        formData
      );
      if (allRoles.statusCode == 200) {
        this.$message.success("修改成功!");
      } else {
        this.$message.success("修改失败！" + listData.message);
      }
      that.editRolesVisible=false;
      that.getData();
    ...
    async editRoles(row) {
      console.log(row);
      this.row = row;
      for (let index = 0; index < row.roles.length; index++) {
        console.log(this.checkRolesList);

        this.checkRolesList.push(row.roles[index].id);
      }
      console.log("this.checkRolesList");
      ...
      let allRoles = await this.$fetch(that.$path + "users/role/");
      if (allRoles.statusCode == 200) {
      this.allRoles = allRoles.dataInfo.listData;
    ...
    async deleteRow() {
      ...
      let listData={};
      if (that.delType === "del") {
         listData= await this.$del(
          that.$path + "users/users/" + that.row.id + "/"
        );
      } else if (that.delType === "allDel") {
        ...
        console.log(delList);
        that.loading = true;
        listData = await this.$post(that.$path + "users/users/", {
          ids: delList
        });
